...
public class DashboardView extends Div {
	...
	private Div constructSystemAdminDashboard() {
		...
		companiesItem.getContentLeft().add(getSpan("TOTAL COMPANIES", TITLE_SPAN));
		...
		usersItem.getContentLeft().add(getSpan("TOTAL USERS", TITLE_SPAN));
	...
	private Div constructCompanyAdminDashboard() {
		...
		long userCount = UserFacade.getInstance().getUsersInCompany(currentCompany.getId()).size();
		...
		// TOOLS IN COMPANY
		List<Tool> tools = InventoryFacade.getInstance().getAllToolsInCompany(currentCompany.getId());
