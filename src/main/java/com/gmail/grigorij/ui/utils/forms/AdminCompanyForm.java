...
//		country.setWidth("100%");
		...
		companyLocationsComboBox.setItems(company.getLocations());
		companyLocationsComboBox.setItemLabelGenerator(Location::getLocationName);
		companyLocationsComboBox.setWidth("100%");

		Button newLocationButton = UIUtils.createButton("Add New");
		newLocationButton.getStyle().set("display", "table");
		newLocationButton.getStyle().set("margin-left", "10px");
		newLocationButton.addClickListener(e -> openNewLocationDialog());

		FlexBoxLayout locationsLayout = new FlexBoxLayout();
		locationsLayout.setWidth("100%");
		locationsLayout.add(companyLocationsComboBox);
		locationsLayout.add(newLocationButton);
...
//		addFormItem(country, "Country");
		addFormItem(locationsLayout, "Locations");
		addFormItem(new Divider("1px"), "");
...
//				.withValidator(new EmailValidator("Email address invalid"))
		...
		companyBinder.forField(companyLocationsComboBox)
				.bind(Company::getLocation, Company::setLocation);
...
//				.bind(Address::getCountry, Address::setCountry);
	...
	private void openNewLocationDialog() {
//		CustomDialog dialog = new CustomDialog();
//		dialog.setHeader(UIUtils.createH4Label("New Location"));
//		dialog.setConfirmButton(UIUtils.createButton("Create", ButtonVariant.LUMO_SUCCESS, ButtonVariant.LUMO_TERTIARY));
//
//
//		dialog.setContent(
//				new Span("Input new location information"),
//
//		);
//
//		dialog.getConfirmButton().addClickListener(e -> {
//			if (CompanyFacade.getInstance().remove(selectedCompany)) {
//				UIUtils.showNotification("New location added successfully", UIUtils.NotificationType.SUCCESS);
//			} else {
//				UIUtils.showNotification("New location add failed", UIUtils.NotificationType.ERROR);
//			}
//			dialog.close();
//		});
//		dialog.open();
	...
	public void setCompany(Company c) {
		...
		locationForm.setLocation(company.getPersonLocation());
//		locationBinder.readBean(company.getPersonLocation());
	...
	public Company getCompany() {
		try {
			...
			if (companyBinder.isValid()) {
				...
				Location companyLocation = locationForm.getLocation();
//				locationBinder.writeBean(companyLocation);
				company.setPersonLocation(companyLocation);
